...
async function guardarUmbrales() {
  ...
  await apiFetch(`/api/threshold/${formUmbrales.productoId}`, {
    method: 'PUT',
    body: JSON.stringify({
...
<template>
  <section class="space-y-8">
    ...
    <!-- Productos -->
    <DataTable
      :headers="['ID', 'Nombre', { label: 'Precio', class: 'text-right' }, { label: 'Alertas', class: 'text-center' }, '']"
      :items="productos"
      empty="Sin productos."
    >
      <template #row="{ item: p }">
    ...
    <div class="space-y-3">
      ...
      <DataTable
        :headers="['Producto', { label: 'Descuento', class: 'text-right' }, 'Vigencia', '']"
        :items="ofertas"
        empty="Sin ofertas."
      >
        <template #row="{ item: o }">
          ...
          <td class="px-4 py-3 text-right">
            <Badge color="emerald">{{ o.porcentaje }}%</Badge>
          </td>
          <td class="px-4 py-3 text-xs text-slate-500">{{ fecha(o.validoDesde) }} → {{ fecha(o.validoHasta) }}</td>
          <td class="px-4 py-3 text-right">
