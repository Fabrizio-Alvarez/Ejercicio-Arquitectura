...
describe('Oferta value object', function () {
    ...
    it('applies its discount to a price', function () {
        ...
        expect($offer->applyTo(new Dinero(1000, 'ARS'))->amount())->toBe(750);
    });

    it('covers only its own product', function () {
        $offer = new Oferta('p-1', 10.0, new \DateTimeImmutable('2026-01-01'), new \DateTimeImmutable('2026-01-31'));
    ...
    it('rejects a percent outside the 0..100 range', function () {
        expect(fn () => new Oferta('p-1', 150.0, new \DateTimeImmutable('2026-01-01'), new \DateTimeImmutable('2026-01-31')))
            ->toThrow(\InvalidArgumentException::class);

        expect(fn () => new Oferta('p-1', -5.0, new \DateTimeImmutable('2026-01-01'), new \DateTimeImmutable('2026-01-31')))
            ->toThrow(\InvalidArgumentException::class);
    });

    it('rejects a validity window where validTo is earlier than validFrom', function () {
        expect(fn () => new Oferta(
            'p-1',
            10.0,
